...
export function calculatorFactory(options) {
  ...
  return function calculator(inputData) {
    ...
    if(mergedQuestions) inputData.questionAnswers = mapMergedQuestionAnswersToOriginal(inputData.questionAnswers);

    const result = calculatorOriginal(inputData);

    if(mergedQuestions && result.question) result.question = getQuestion(result.question.key);
